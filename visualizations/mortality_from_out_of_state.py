...
# state as a result of emissions occurring outside that state
...
def main():
    ...
    print(sum(states["mort_from_out_state"]))

    # plot chloropleth
    states.plot(column='mort_from_out_state',
                cmap="inferno_r",
                legend=True)

    plt.title('Annual Premature Mortality from out-of-State Sources', fontsize=10)
